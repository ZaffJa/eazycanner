...
if( !function_exists('tlg_framework_gmap_shortcode') ) {
	function tlg_framework_gmap_shortcode( $atts, $content = null ) {
		...
		get_option( 'tlg_framework_gmaps_key', '' ) : 'AIzaSyDvzN6liGLD0Mf1iSyHkcbga54nu8Xx9u0';
		...
		return '<div id="' . esc_attr($map_id) . '" style="height: ' . esc_attr($height) . 'px;"></div>
	...
	function tlg_framework_gmap_shortcode_vc() {
		vc_map( array(
			...
			'params' 		=> array(
				array(
					'type'        	=> 'textarea',
					'heading'     	=> esc_html__( 'Map locations', 'tlg_framework' ),
					'param_name'  	=> 'locations',
					'admin_label' 	=> true,
					'description' 	=> esc_html__( 'Please enter the the list of locations you would like. Divide values with linebreaks (Enter).', 'tlg_framework' )
				),
				array(
					'type' 			=> 'tlg_number',
				...
				array(
					...
					'param_name' => 'style',
					'value'      => array(
						esc_html__( '(default style)', 'tlg_framework' ) 	=> '',
