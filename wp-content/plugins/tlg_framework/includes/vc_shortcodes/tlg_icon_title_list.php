...
if( !function_exists('tlg_framework_icon_title_list_shortcode') ) {
	function tlg_framework_icon_title_list_shortcode( $atts, $content = null ) {
		...
		return '<div class="icon_title_list-content '. esc_attr($style) .'"><ul class="icon_title_list">'. do_shortcode($content) .'</ul></div>';
	}
...
if( !function_exists('tlg_icon_title_list_content_shortcode') ) {
	function tlg_icon_title_list_content_shortcode( $atts, $content = null ) {
		...
		$output .= $icon ?  '<i '.$style_icon.' class="'.esc_attr($icon).' '.esc_attr($icon_layout).' inline-block icon"></i>' : '';
		$output .= '</span><span>'. htmlspecialchars_decode($title) .'</span></li>';
		return $output;
	}
